...
export default function CommandPaletteInner({ onClose }: { onClose: () => void }) {
    ...
    const run = useCallback((action: () => void) => { onClose(); action(); }, [onClose]);

    const handleCopy = useCallback((type: "email" | "link") => {
        const text = type === "email" ? CONTACT_EMAIL : window.location.href;
    ...
    return (
        <div className={containerClass}>
            <div className="absolute inset-0 bg-black/60 backdrop-blur-md animate-in fade-in duration-150" onClick={onClose} />
